...
the comoving coordinates.  Two systems in this coordinate system
...
so we have that
\begin{equation}
\int_{t_e+\Delta t_e}^{t_0 + \Delta t} \frac{dt}{R(t)} = \int_{t_e}^{t_0} \frac{dt}{R(t)},~~\mathrm{so}~\frac{\Delta t_e}{R(t)} = \frac{\Delta t}{R(t_0)},
\end{equation}
\noindent
provided that $\Delta t \ll R(t)/\dot{R}(t)$.  So all processes appear to be
